...
function Login() {
  ...
  const portalTitle = portal === 'empresa' ? 'Empresa' : 
                     portal === 'coordinador' ? 'Coordinador/a' : 
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (location.pathname.includes('inspector')) {
        ...
        // Store inspector username for later use
        localStorage.setItem('inspectorUsername', formData.email);
        ...
        <div className="text-center mb-8">
          ...
          <h2 className="text-2xl font-bold text-gray-900">
            Portal {isSupervisorDelegation ? 'Supervisor Delegación' : 
                   isSupervisorCenter ? 'Coordinador/a' : 
                   portalTitle}
          </h2>
