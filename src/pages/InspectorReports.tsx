...
export default function InspectorReports() {
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      try {
        ...
        if (!inspectorData || !inspectorData.company_id) {
          throw new Error('Inspector no asociado a ninguna empresa');
        }

        console.log('Inspector company_id:', inspectorData.company_id);
        setCompanyId(inspectorData.company_id);

        await fetchWorkCenters(inspectorData.company_id);
        await fetchEmployees(inspectorData.company_id);
        await fetchCompanyInfo(inspectorData.company_id);
  ...
  const fetchWorkCenters = async (companyId: string) => {
    try {
  ...
  const fetchTimeEntriesInBatches = async (employeeIds: string[], startDate?: string, endDate?: string) => {
    if (!employeeIds.length || !companyId) return [];
    ...
    for (const batch of batches) {
      try {
        // First get employee_ids that belong to the company
        ...
        // Then get time entries only for those employees
        let query = supabase
          .from('time_entries')
          .select('*')
  ...
  const fetchCompanyTimeRequests = async () => {
    try {
      ...
      const { data, error } = await supabase
        ...
        `)
        .eq('employee_profiles.company_id', companyId)
  ...
  const generateReport = async () => {
    ...
    try {
      if (!companyId) {
        throw new Error('No se ha identificado la empresa asociada');
      }
  ...
  // The rest of the component (UI) remains exactly the same as CompanyReports
  return (
    <div className="p-8">
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          <h1 className="text-2xl font-bold mb-2">Informes del Inspector</h1>
